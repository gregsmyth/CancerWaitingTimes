...
header <- list("DATE","ODS_CODE","PROVIDER","CARE_SETTING", "CANCER_TYPE",	"TOTAL",	"WITHIN_62_DAYS",	"AFTER_62_DAYS", "PERCENTAGE_TREATED_WITHIN_62_DAYS", "EMPTY", "WITHIN_31_DAYS", "FROM_32_TO_38_DAYS", "FROM_39_TO_48_DAYS", "FROM_49_TO_62_DAYS", "FROM_63_TO_76_DAYS", "FROM_77_TO_90 DAYS",	"FROM_91_TO_104_DAYS", "AFTER_104_DAYS")

colClasses = c("integer", "character", "character","character", "character","integer", "integer","integer", "character","integer","integer","integer","integer","integer","integer","integer","integer","integer")
...
for (thisMonth in c(201506,201507,201508,201509,201510,201511,201512,201601,201602,201603,201604,201605,201606,201607,201608,201609,201610,201611,201612,201701,201702,201703,201704,201705,201706,201707)) {
  #thisMonth <- 201707
  thisMonthFile <- paste("./Data/", thisMonth,"-11.-62-Day-Wait-for-first-treatment-All-Cancers-Provider-Data.csv", sep = "")
...
allMonthsAllCancer$PERCENTAGE_TREATED_WITHIN_62_DAYS <- clean(allMonthsAllCancer$PERCENTAGE_TREATED_WITHIN_62_DAYS)
...
allMonthsAllCancer[, 10:17] <- sapply(allMonthsAllCancer[, 10:17], as.integer)
...
for (thisProvider in allProviders) {
  ...
  thisMean <- mean(providerData$PERCENTAGE_TREATED_WITHIN_62_DAYS)
  ucl <- mean(providerData$PERCENTAGE_TREATED_WITHIN_62_DAYS) + 3*sd(providerData$PERCENTAGE_TREATED_WITHIN_62_DAYS)
  lcl <- mean(providerData$PERCENTAGE_TREATED_WITHIN_62_DAYS) - 3*sd(providerData$PERCENTAGE_TREATED_WITHIN_62_DAYS)
  
  filename <- paste("./62-Day-Wait/", thisProvider, ".eps", sep = "") 
  
  ggplot(providerData, mapping = aes(x=DATE,y=PERCENTAGE_TREATED_WITHIN_62_DAYS, group=1)) +
    geom_line() +
    geom_point() +
